...
timeline.fromTo(
  ...
  0.2,
  {},
  { visibility: 'visible', ease: Power2.easeInout },
  '-=0.2'
);
...
toggleMenu
  .to(line_2, 0.125, { scaleX: 0 })
  .to(
    line_1,
    0.125,
    { rotation: 45, transformOrigin: '50% 50%', y: 8 },
    '+=.125'
  )
  .to(
    line_3,
...
// })
const aside_prevent = document.querySelector('.depth2 > li > a');
const response_768 = window.matchMedia('screen and (max-width: 767px)');
response_768.addListener(() => {
  aside_prevent.addEventListener('click', (e) => {
    e.preventDefault();
  });
});
